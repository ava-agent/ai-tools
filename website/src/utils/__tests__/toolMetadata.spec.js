...
describe('toolMetadata', () => {
  ...
  it('tracks official-source metadata for the first high-impact verification wave', () => {
    ...
    Object.entries(expectedSources).forEach(([id, sources]) => {
      ...
      expect(tool?.lastVerified).toBe('2026-06-25')
  ...
  it('tracks official-source metadata for the second coding-tool verification wave', () => {
    ...
    Object.entries(expectedSources).forEach(([id, sources]) => {
      const tool = aiToolsData.find((item) => item.id === id)
      const expectedLastVerified = id === 'deepseek' ? '2026-07-02' : '2026-06-26'

      expect(tool?.verificationStatus).toBe('verified')
      expect(tool?.lastVerified).toBe(expectedLastVerified)
      expect(tool?.decisionSummary?.verdict).toBeTruthy()
      expect(tool?.decisionSummary?.alternatives?.length).toBeGreaterThan(0)
      sources.forEach((source) => {
        expect(tool?.sources).toContain(source)
  ...
  it('tracks official-source metadata for the priority multimodal verification wave', () => {
    const expected = {
      midjourney: {
        ...
        mustInclude: ['V7', '$120', 'niji'],
        forbidden: /MJ v6|V6\.1|\$10-60|需要 Discord/,
      },
      'stable-diffusion': {
        sources: [
      ...
      kling: {
        ...
        mustInclude: ['Kling 3.0', 'Resource Packages', 'Text-to-Video'],
      ...
      runway: {
        ...
        mustInclude: ['Gen-4.5', '12 credits per second', '125 credits'],
        forbidden: /Gen-3 Alpha|\$12\/\$28\/\$76|业界领先/,
      },
      suno: {
        sources: [
        ...
        mustInclude: ['v5.5', '50 credits', '10 songs per day'],
      ...
      elevenlabs: {
        ...
        mustInclude: ['Eleven v3', '10,000 credits', 'commercial license'],
    ...
    Object.entries(expected).forEach(([id, { sources, mustInclude, forbidden }]) => {
      ...
      expect(tool?.verificationStatus).toBe('verified')
      expect(tool?.lastVerified).toBe('2026-07-01')
      expect(tool?.decisionSummary?.verdict).toBeTruthy()
  ...
  it('tracks official-source metadata for the remaining coding-tool verification wave', () => {
    ...
    Object.entries(expected).forEach(([id, { status, sources }]) => {
      const tool = aiToolsData.find((item) => item.id === id)
      const text = JSON.stringify(tool)

      expect(tool?.verificationStatus).toBe(status)
      expect(tool?.lastVerified).toBe('2026-06-29')
      expect(tool?.decisionSummary?.verdict).toBeTruthy()
      expect(tool?.decisionSummary?.mainRisk).toBeTruthy()
      sources.forEach((source) => {
        expect(tool?.sources).toContain(source)
    ...
    expect(JSON.stringify(qwenCli)).not.toMatch(/2000\u6b21\/\u5929|1000\u6b21\/\u5929|60\u6b21\/\u5206\u949f/)
  ...
  it('tracks official-source metadata for the continued LLM verification wave', () => {
    ...
    Object.entries(expected).forEach(([id, { sources, mustInclude }]) => {
      const tool = aiToolsData.find((item) => item.id === id)
      const text = JSON.stringify(tool)
    ...
    expect(JSON.stringify(aiToolsData.find((item) => item.id === 'doubao'))).not.toMatch(/Doubao-Seed-Code/)
  ...
  it('tracks official-source metadata for the remaining LLM verification wave', () => {
    const expected = {
      qinglizi: {
        ...
        mustInclude: ['App 内购买', '当前商店页及产品内为准', '不作为诊断、确诊和治疗依据', '不能代替医生面诊', '医疗行业专业用户参考'],
      ...
      llama: {
        ...
        mustInclude: ['Llama 4 Scout', 'Llama 4 Maverick', '10M', '1M'],
        forbidden: /\u5b8c\u5168\u514d\u8d39|\u5b8c\u5168\u5f00\u6e90|EU \u5730\u533a\u7981\u6b62|AI Act/,
      },
      mistral: {
        status: 'verified',
        sources: [
      ...
      yi: {
        ...
        mustInclude: ['Yi-Lightning', 'Yi-Coder', 'Apache 2.0', 'Billing', 'real-name authentication', 'limited trial purposes only', 'without use of Yi Model or Outputs in production', 'input and generated content'],
      ...
      ernie: {
        ...
        mustInclude: ['ERNIE 5.0', 'ERNIE 4.5 Turbo', 'ERNIE X1.1'],
        forbidden: /\u6d88\u8d39\u8005\u5b8c\u5168\u514d\u8d39|\u8d85\u8d8a GPT-4\.5|ERNIE 5\.0 \u4e8e 2025/,
      },
      cohere: {
        status: 'verified',
        sources: [
      ...
      'amazon-nova': {
        ...
        mustInclude: ['Amazon Nova Premier', 'Nova Pro', 'Nova Lite', 'Nova Micro'],
      ...
      hunyuan: {
        ...
        mustInclude: ['hunyuan-a13b', 'Tencent HY Vision 1.5', 'TokenHub'],
        forbidden: /\u514d\u8d39\(腾讯云试用\)|Turbo\/Pro\/Lite/,
      },
    }

    Object.entries(expected).forEach(([id, { status, lastVerified = '2026-07-01', sources, mustInclude, forbidden }]) => {
      const tool = aiToolsData.find((item) => item.id === id)
      const text = JSON.stringify(tool)
  ...
  it('tracks official-source metadata for the continued developer-tool verification wave', () => {
    const expected = {
      zed: {
        ...
        mustInclude: ['Personal', '$10', 'Business', '2,000 accepted edit predictions'],
      ...
      tabnine: {
        ...
        mustInclude: ['Code Assistant', '$39', 'Agentic Platform', '$59'],
        forbidden: /\$12|\u514d\u8d39\u7248\u57fa\u7840\u4ee3\u7801\u8865\u5168|\$39\u7528\u6237\u6708\(Enterprise\)/,
      },
      warp: {
        sources: [
        ...
        mustInclude: ['Build', '$20', '1,500 credits', 'BYOK'],
      ...
      crush: {
        ...
        mustInclude: ['Multi-Model', 'LSP', 'MCP', '100 Hypercredits'],
    ...
    Object.entries(expected).forEach(([id, { sources, mustInclude, forbidden }]) => {
      ...
      expect(tool?.verificationStatus).toBe('verified')
      expect(tool?.lastVerified).toBe('2026-07-02')
      expect(tool?.decisionSummary?.verdict).toBeTruthy()
      expect(tool?.decisionSummary?.mainRisk).toBeTruthy()
      sources.forEach((source) => {
        expect(tool?.sources).toContain(source)
  ...
  it('tracks official-source metadata for the priority MCP verification wave', () => {
    const expected = {
      context7: {
        ...
        mustInclude: ['Free for personal use', 'Pro', '$10', '5,000 free API calls'],
      ...
      'github-mcp': {
        ...
        mustInclude: ['OAuth', 'PAT', 'toolsets', 'read-only'],
  ...
  it('tracks official-source metadata for the agent framework verification wave', () => {
    const expected = {
      n8n: {
        ...
        mustInclude: ['20€/mo', '50€/mo', 'Community Edition', 'Sustainable Use License'],
      ...
      'openai-agents-sdk': {
        ...
        mustInclude: ['provider-agnostic', '100+ other LLMs', 'Handoffs', 'Guardrails', 'Tracing'],
      ...
      'claude-agent-sdk': {
        ...
        mustInclude: ['Claude Code as a library', 'Python and TypeScript', 'Python 3.10+', '@anthropic-ai/claude-agent-sdk'],
      ...
      'google-adk': {
        ...
        mustInclude: ['Python, TypeScript, Go, Java, and Kotlin', 'Code Execution', 'Google Search', 'Apache 2.0'],
  ...
  it('tracks official-source metadata for the AI IDE cleanup verification wave', () => {
    const expected = {
      codebuddy: {
        ...
        mustInclude: ['$9.95/month', '1,000 credits', 'Team Plan', 'OpenAI interface format'],
      ...
      antigravity: {
        ...
        mustInclude: ['Google AI Pro', 'Google AI Ultra', '$99.99', '$199.99', 'highest usage limits'],
      ...
      verdent: {
        ...
        mustInclude: ['7 天 100 credits', 'Eco Mode', 'BYOK', '76.1% pass@1', 'provider pricing'],
        forbidden: /Free 7天100 credits\/Starter \$19\/月\/Pro \$59\/月\/Max \$179\/月|SWE-bench Verified 76\.1% pass@1|证明其实力/,
      },
    }
  ...
  it('tracks official-source metadata for the agent and creative-tool verification wave', () => {
    const expected = {
      manus: {
        ...
        mustInclude: ['Manus 1.6', '$20/month', '300 credits', '4,000 credits per month', 'Team plan'],
        forbidden: /订阅制（具体定价待定）|等待名单近百万|2025 年 12 月|20亿|MIT Technology Review|不建议专业业务使用|年化收入/,
      },
      crewai: {
        sources: [
        ...
        mustInclude: ['Basic Free', '50 workflow executions/month', 'Enterprise Custom', 'Crews', 'Flows'],
      ...
      flux: {
        ...
        mustInclude: ['FLUX.2', '1 credit = $0.01 USD', 'from $0.014', 'up to 4MP', 'Apache2.0'],
      ...
      pika: {
        ...
        mustInclude: ['Pika 2.5', '80 monthly video credits', '$8', '$28', '$76', 'Pikascenes'],
        forbidden: /免费版每日有限 credits|已接近 Runway 水平|性价比在视频生成工具中数一数二|免费\/\$8\/\$28\/月/,
      },
      jimeng: {
        sources: [
        ...
        mustInclude: ['图片生成4.0', '图片生成4.6', '视频生成3.0 Pro', '积分/会员', '以订单结算页价格为准'],
        forbidden: /¥69月|每日 80-100 积分|中文提示词支持最佳|质量已接近 Midjourney|极为慷慨|12种电影级/,
      },
      luma: {
        sources: [
        ...
        mustInclude: ['$30/月', '$90/月', '$300/月', 'Ray3.2', 'Luma Agents', '$4 起可购 1,200 credits'],
        forbidden: /免费\(30代\/月\)|\$29\/\$99\/\$499|Dream Machine 2\.0|与 Sora 相比生成速度更快|免费额度也更友好/,
      },
      udio: {
        sources: [
        ...
        mustInclude: ['每日 10 credits', '100 credits', 'Standard', '2,400 credits', 'Pro', '6,000 credits', '$10/月', '$30/月'],
  ...
  it('tracks official-source metadata for the knowledge and agent platform verification wave', () => {
    const expected = {
      notebooklm: {
        ...
        mustInclude: ['100 notebooks', '50 sources', '500,000 words', '50 chat queries', '3 audio generations', 'Google AI Pro'],
      ...
      coze: {
        ...
        mustInclude: ['10 credits/day', '100 credits/day', '$19', '$39', '3-day free trial'],
      ...
      dify: {
        ...
        mustInclude: ['Sandbox Free', '200 message credits', '$59', '$159', 'Community Edition'],
      ...
      langchain: {
        ...
        mustInclude: ['LangSmith', '$0 / seat', '$39 / seat', 'LangGraph', 'MIT-licensed', '100M+'],
      ...
      perplexity: {
        ...
        mustInclude: ['$20/month', '$200/monthly', '$40 per month', '$325 per month', 'cited answers', 'Comet Browser'],
      ...
      autogen: {
        ...
        mustInclude: ['maintenance mode', 'Python 3.10+', 'autogen-agentchat', 'v0.4', 'Magentic-One'],
  ...
  it('tracks official-source metadata for the high-risk MCP verification wave', () => {
    const expected = {
      'brave-search-mcp': {
        ...
        mustInclude: ['$5 in free monthly credits', '$5 per 1,000 requests', '$4 per 1,000 requests', 'BRAVE_MCP_TRANSPORT', 'brave_llm_context'],
      ...
      'supabase-mcp': {
        ...
        mustInclude: ['read_only=true', 'project_ref', 'features=database,docs', 'never connect the MCP server to production data', 'pre-1.0'],
      ...
      'stripe-mcp': {
        ...
        mustInclude: ['https://mcp.stripe.com', 'OAuth', 'restricted API keys', 'sandbox and live mode', 'human confirmation', 'stripe_api_write'],
        forbidden: /anthropics\/claude-code|Stripe\/Community|完全免费|只用 test mode API Key|避免 AI 自动操作支付/,
      },
      'cloudflare-mcp': {
        sources: [
        ...
        mustInclude: ['over 2,500 endpoints', 'search()', 'execute()', 'OAuth', 'API token', 'Dynamic Worker sandbox'],
      ...
      'slack-mcp': {
        ...
        mustInclude: ['secure access to your Slack workspace', 'search messages', 'send messages', 'OAuth', 'scopes', 'read specific channels or threads'],
      ...
      'postman-mcp': {
        ...
        mustInclude: ['Minimal', 'Code', 'Full', '100+', 'OAuth', 'EU remote server', 'API key', 'https://mcp.postman.com/minimal'],
      ...
      'sentry-mcp': {
        ...
        mustInclude: ['OAuth', 'Auth Token', 'permissions and scopes', 'production error data', 'PII', 'project scopes'],
      ...
      'docker-mcp': {
        ...
        mustInclude: ['Availability: Beta', '隔离 Docker 容器', '限制权限', '网络访问', '资源使用', 'logging', 'call-tracing', '1 CPU', '2 GB'],
  ...
  it('tracks current metadata for memory, collaboration, and automation platforms', () => {
    const expected = {
      'openmemory-mcp': {
        ...
        mustInclude: ['cloud-hosted MCP server requires no local installation', 'Mem0 Platform account', 'API key', '9 memory tools', 'archived', 'MEM0_API_KEY', 'memory persistence risk'],
      ...
      'atlassian-mcp': {
        ...
        mustInclude: ['Atlassian Rovo MCP Server', 'Generally Available', 'OAuth 2.1 or API tokens', 'existing access controls', 'Beta tools', 'Rovo credits', 'Cloud and Server/Data Center deployments', 'API rate limits'],
        forbidden: /完全免费|企业级团队协作利器|适合生产环境|只读模式保护生产数据|官方 MCP 出现|无缝|效率利器/,
      },
      'zapier-ai': {
        sources: [
        ...
        mustInclude: ['9,000+ apps', '100 tasks per month', '$19.99/month', 'task-based pricing', 'AI steps, code, and SDK', 'Zapier MCP Beta', 'shared task pool', 'Each successful action'],
        forbidden: /7000\+|\$29\.99|8M\+|老牌王者|应用集成数量最多|企业级可靠性和安全性|复杂流程建议用 n8n|AI Actions API/,
      },
      make: {
        sources: [
        ...
        mustInclude: ['1,000 credits/month', '$9/mo', '$16/mo', '$29/mo', '3000+ apps', '350+ AI apps', 'Make MCP Server', 'credits 是计费单位', '使用 Make AI Provider 的部分高级功能可能消耗更多'],
        forbidden: /\$10\.59|2000\+|性价比高于 Zapier|比 Zapier 更灵活|免费版给 1000 操作\/月比 Zapier 大方|知名度不如 Zapier|操作\/月/,
      },
    }
  ...
  it('tracks current metadata for collaboration, deployment, and data MCP platforms', () => {
    const expected = {
      'notion-mcp': {
        ...
        mustInclude: ['hosted server', 'read and write', 'access and permissions', 'standard OAuth', 'remote MCP', 'local MCP server repository', 'not actively monitored'],
      ...
      'figma-mcp': {
        ...
        mustInclude: ['remote server is available on all seats and plans', 'desktop server is available on a Dev or Full seat', 'usage-based paid feature', 'currently available for free during the beta period', 'https://mcp.figma.com/mcp', 'write native Figma content'],
      ...
      'vercel-mcp': {
        ...
        mustInclude: ['Beta', 'all plans', 'https://mcp.vercel.com', 'OAuth', 'approved by Vercel', 'public tools', 'authenticated tools', 'get_runtime_logs', 'human confirmation', 'prompt injection'],
      ...
      'linear-mcp': {
        ...
        mustInclude: ['https://mcp.linear.app/mcp', 'OAuth token', 'API keys', 'Authorization: Bearer', 'Free', '250 issues', '$10 per user/month', '$16 per user/month', 'MCP access', 'AI credits'],
        forbidden: /完全免费|anthropics\/claude-code|Linear\/Community|最受开发者欢迎|避免频繁切换|修 Bug → 更新 Issue/,
      },
      'gitlab-mcp': {
        sources: [
        ...
        mustInclude: ['OAuth 2.0 Dynamic Client Registration', 'HTTP transport', '/api/v4/mcp', 'Premium, Ultimate', 'Status: Beta', 'perform actions on your behalf', 'personal access tokens', 'Rotate', 'Revoke'],
        forbidden: /完全免费|anthropics\/claude-code|GitLab\/Community|支持自托管实例是相对 GitHub MCP 的独特优势|一应俱全/,
      },
      'elasticsearch-mcp': {
        sources: [
        ...
        mustInclude: ['two MCP server options', 'Agent Builder MCP server', 'recommended approach for Elasticsearch 9.2+', 'Serverless projects', 'KIBANA_URL', 'API_KEY', 'scope assigned to the API key', 'least privilege'],
        forbidden: /完全免费|Elastic\/Community|anthropics\/claude-code|自然语言转 ES 查询|不用记 DSL|非常实用|聚合查询结果可视化/,
      },
    }
  ...
  it('tracks current metadata for multimodal creator tools with pricing and credit risks', () => {
    const expected = {
      'meitu-ai': {
        ...
        mustInclude: ['AI 消除', 'AI 抠图', '照片扩展', '美图 VIP', '美图 VIP+', '美图 PC VIP', 'AI Agent VIP', '订阅权益'],
        forbidden: /免费版功能已经很够用|VIP 价格也很亲民|完全不需要设计基础|电商、社交媒体运营|非开发者工具/,
      },
      'jianying': {
        sources: [
        ...
        mustInclude: ['价格会因地区变化', '月付和年付订阅', 'AI points 从 550 提升到 1200', '云存储从 100GB 提升到 1TB', 'Standard Plan', 'CapCut Online'],
        forbidden: /免费版功能强大|与抖音深度集成|国内短视频创作者的标配|智能字幕识别准确率极高|免费版功能已非常强大/,
      },
      'hailuo': {
        sources: [
        ...
        mustInclude: ['Hailuo 2.3', '网站、移动 app 和 Open Platform API', '每日免费 trial credits', 'Membership Credits', 'Purchased Credits', '$1 可购买 70 credits', 'MiniMax-Hailuo-2.3-Fast', '768P、6 秒视频 $0.19'],
        forbidden: /免费\(每日额度\)|每日免费额度|免费使用是最大亮点|国际知名度较低|人物动态表现自然/,
      },
      'ideogram': {
        sources: [
          'https://ideogram.ai/',
        ...
        mustInclude: ['Ideogram 4.0', 'Free Plan', 'Plus Plan', 'Pro Plan', 'Team Plan', '10 credits/week', '$20 USD per month', '$60 USD per month', 'Priority credits', 'API Beta', '自动充值', '不限制用户对输出内容的权利'],
        forbidden: /\$8\/\$20\/月|Ideogram 2\.0\/3\.0|免费版每日有限生成|每天可生成约 10 张图|\$8\/月专业版性价比高|文字渲染能力业界最强/,
      },
      'meshy': {
        sources: [
        ...
        mustInclude: ['$0', '$20/月', '$60/月', '每月 100 credits', 'CC BY 4.0', '每月 1,000 credits', '每月重置且不累积', 'Text to 3D（Preview）', 'Image to 3D', 'Meshy-6'],
      ...
      'hunyuan3d': {
        ...
        mustInclude: ['每日 20 次免费生成', '200 免费 credits', 'Hunyuan 3D 3.0', 'Hunyuan3D 2.0', 'Hunyuan3D-2.1 Series', 'text-to-3D', 'image-to-3D', 'Sketch-to-3D', 'OBJ 和 GLB', '6GB', '16GB'],
        forbidden: /免费\/低价|有免费额度|中文提示词理解最佳|腾讯生态集成潜力大|导出格式有限/,
      },
    }
  ...
  it('tracks current metadata for agent frameworks, MCP utilities, and Gemini image generation', () => {
    const expected = {
      autogpt: {
        ...
        mustInclude: ['Pro', '$42.50/月', 'Max', '$272.00/月', 'Self-Host 开源', '可视化 Agent Builder', 'MCP Tool Support', 'Polyform Shield License', 'MIT License'],
      ...
      nanobanana: {
        ...
        mustInclude: ['Nano Banana 2', 'Gemini 3.1 Flash Image', 'Nano Banana Pro', 'Gemini 3 Pro Image', 'gemini-3.1-flash-image', 'gemini-3-pro-image', 'SynthID', '可见水印', 'Create images'],
        forbidden: /图表生成|https:\/\/nanobanana\.google|完全免费|无需注册|架构图|流程图|ER 图|技术文档配图/,
      },
      'sequential-thinking-mcp': {
        ...
        mustInclude: ['sequential_thinking', '@modelcontextprotocol/server-sequential-thinking', 'npx -y', 'Docker', 'nextThoughtNeeded', 'isRevision', 'branchFromThought', 'host 会多次调用该工具', 'MIT License'],
        forbidden: /Anthropic \(MCP 官方\)|思维链构建透明可观测|显著提升|杀鸡用牛刀|Claude Code 内置深度思考能力|完全免费/,
      },
      'antv-chart-mcp': {
        sources: [
        ...
        mustInclude: ['@antv/mcp-server-chart', '25+ 图表类型', 'generate_word_cloud_chart', 'generate_spreadsheet', 'Dify', 'AMap 服务', '只支持中国范围地图生成', 'chart-visualization'],
        forbidden: /完全免费|无需LLM|质量有保障|输出格式美观|SDK 支持程序化调用|图表交互性有限|国际化文档较少/,
      },
      'camel-ai': {
        sources: [
        ...
        mustInclude: ['pip install camel-ai', 'ChatAgent', 'Agent Societies', 'Data Generation', 'Tools', 'Memory', 'Human-in-the-Loop', 'Apache 2.0'],
        forbidden: /首个 LLM 多 Agent 框架|学术界广泛引用|生产部署经验有限|文档不够工程化|工程化程度不如|完全免费/,
      },
      agentscope: {
        sources: [
        ...
        mustInclude: ['AgentScope 2.0', 'Python 3.11 或更高版本', 'uv pip install agentscope', 'Event System', 'Permission System', 'Multi-tenancy & Multi-session Service', 'Workspace / Sandbox Support', 'Apache License 2.0'],
        forbidden: /微软亚研院出品|质量有保障|中文文档完善|国产 Agent 框架的领先者|国际知名度不如 LangChain|完全免费/,
      },
    }
  ...
  it('tracks public-source metadata for security skill entries with runtime caveats', () => {
    const expected = {
      'semgrep-skill': {
        ...
        mustInclude: ['semgrep/skills', '需要单独安装', '需要 Semgrep CLI', 'Semgrep Community Edition', 'Semgrep Code', '轻量 SAST', 'LGPL 2.1', '人工复核'],
        forbidden: /Claude Code 内置|预装|完整 Semgrep 静态分析扫描|自动检测项目语言并选择规则集|并行 Worker|极其有价值|锦上添花|完全免费/,
      },
      'differential-review-skill': {
        ...
        mustInclude: ['Trail of Bits', '需要单独安装', '需要 Git 仓库', 'PR、commit 或 diff', 'blast radius', 'test coverage', '不会批准或阻止 PR', '缺少提示注入防护证据', '人工复核'],
        forbidden: /安全审计利器|自动计算变更爆炸半径|检测安全回归|测试覆盖率分析|完整 Markdown 审查报告|能发现普通 Code Review 容易遗漏|完全免费/,
      },
    }
  ...
  it('tracks source boundaries for community skill catalog entries', () => {
    const expected = {
      'frontend-design-skill': {
        ...
        mustInclude: ['frontend-app-builder', 'redesign/restyle/modernization', 'Browser/IAB', '真实浏览器', '页面现代化'],
        forbidden: /Claude Code 内置|完全免费|67 种风格|96 种调色板|57 种字体|25 种图表类型|shadcn\/ui MCP 集成/,
      },
      'webapp-testing-skill': {
        status: 'verified',
        sources: [
          '本地核验线索：frontend-testing-debugging',
        ],
        mustInclude: ['frontend-testing-debugging', 'Browser plugin', 'Playwright 降级验证', 'DOM 快照', '控制台健康检查', '交互证明'],
        forbidden: /Claude Code 内置|完全免费|Playwright\/Cypress 集成|测试覆盖率分析|属性测试|快照测试/,
      },
      'mcp-builder-skill': {
        status: 'verified',
        sources: [
          '本地核验线索：building-mcp-server-on-cloudflare',
          'https://modelcontextprotocol.io/docs/learn/architecture',
          'https://modelcontextprotocol.io/specification/2025-06-18/server/tools',
        ],
        mustInclude: ['building-mcp-server-on-cloudflare', 'Cloudflare Workers', 'remote MCP servers', 'MCP architecture', 'tools expose executable functions'],
        forbidden: /Claude Code 内置|完全免费|完整的 MCP 协议支持|TypeScript\/Python 双语言支持|文档自动生成/,
      },
      'skill-creator-skill': {
        status: 'verified',
        sources: [
          '本地核验线索：skill-creator',
        ],
        mustInclude: ['skill-creator', 'SKILL.md', 'scripts/init_skill.py', 'scripts/quick_validate.py', 'agents/openai.yaml', 'progressive disclosure'],
        forbidden: /Claude Code 内置|完全免费|领域专家设计原则|提示词工程最佳实践|技能发布流程指导/,
      },
      'insecure-defaults-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 insecure-defaults skill（检查于 2026-07-02）',
          '本地核验线索：security-best-practices',
        ],
        mustInclude: ['公开来源不足', '未找到可公开核验的同名 insecure-defaults Skill', 'security-best-practices', 'secure-by-default', 'major issues', '人工复核', '不建议继续作为独立 active Skill'],
        forbidden: /Claude Code 内置|完全免费|检测 fail-open 不安全默认值|发现硬编码密钥和弱认证|覆盖 OWASP 常见配置问题|上线前的安全检查必备技能/,
      },
      'sharp-edges-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 sharp-edges skill（检查于 2026-07-02）',
          '本地核验线索：security-best-practices',
        ],
        mustInclude: ['公开来源不足', '未找到可公开核验的同名 sharp-edges Skill', 'API design', 'secure-by-default', 'misuse risk', '人工复核', '不建议继续作为独立 active Skill'],
        forbidden: /Claude Code 内置|完全免费|密码学库安全评估|提供 misuse-resistant 改进建议|独特的 API 安全视角/,
      },
    }
  ...
  it('tracks source boundaries for remaining skill catalog entries', () => {
    const expected = {
      'second-opinion-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 second-opinion skill（检查于 2026-07-02）',
          '本地核验线索：requesting-code-review',
          '本地核验线索：receiving-code-review',
        ],
        mustInclude: ['公开来源不足', '没有同名 Skill', 'requesting-code-review', 'receiving-code-review', '不建议继续作为独立 active Skill'],
        forbidden: /完全免费|Claude Code \+ OpenAI\/Gemini|Codex\/Gemini CLI|真正的"第二意见"代码审查|多模型交叉验证/,
      },
      'audit-context-building-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 audit-context-building skill（检查于 2026-07-02）',
          '本地核验线索：security-threat-model',
          '本地核验线索：security-best-practices',
        ],
        mustInclude: ['公开来源不足', '未找到可公开核验的同名 audit-context-building Skill', 'repository-grounded threat modeling', 'trust boundaries', 'major issues', '不建议继续作为独立 active Skill'],
        forbidden: /完全免费|Claude Code 内置|逐行深度代码分析|漏洞发现前的深度理解|适合大型代码库审计/,
      },
      'git-cleanup-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 git-cleanup skill（检查于 2026-07-02）',
          '本地核验线索：using-git-worktrees',
        ],
        mustInclude: ['公开来源不足', '未找到可公开核验的同名 git-cleanup Skill', 'using-git-worktrees', '隔离工作区', '不建议继续作为独立 active Skill'],
        forbidden: /完全免费|Claude Code 内置|智能分类：已合并\/squash 合并\/已废弃\/活跃|交互式确认清理|Worktree 关联清理/,
      },
      'ask-questions-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 ask-questions-if-underspecified skill（检查于 2026-07-02）',
          '本地核验线索：brainstorming',
          '本地核验线索：Product Design get-context',
        ],
        mustInclude: ['公开来源不足', 'brainstorming', 'get-context', 'one targeted question', '不建议继续作为独立 active Skill'],
        forbidden: /完全免费|Claude Code 内置|结构化的问题收集框架|区分关键问题和可选问题|减少沟通成本/,
      },
      'property-based-testing-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 property-based-testing skill（检查于 2026-07-02）',
          '本地核验线索：test-driven-development',
          '本地核验线索：systematic-debugging',
        ],
        mustInclude: ['公开来源不足', '未找到可公开核验的同名 property-based-testing Skill', 'test-driven-development', 'systematic-debugging', '不建议继续作为独立 active Skill'],
        forbidden: /完全免费|Claude Code 内置|多语言支持（JS\/Python\/Rust\/Solidity）|智能合约 Fuzz Testing|比示例测试覆盖更全面/,
      },
      'supabase-skill': {
        status: 'verified',
        sources: [
          '本地核验线索：supabase',
          '本地核验线索：supabase-postgres-best-practices',
        ],
        mustInclude: ['Supabase changes frequently', 'verify against changelog', 'RLS', 'Supabase CLI', 'MCP server', 'Postgres performance'],
        forbidden: /完全免费|Claude Code 内置|Supabase 全栈开发专家|高级功能需 Supabase Pro/,
      },
      'supabase-realtime-skill': {
        status: 'historical',
        sources: [
          '公开来源不足：未发现同名 supabase-realtime skill（检查于 2026-07-02）',
          '本地核验线索：supabase',
        ],
        mustInclude: ['历史别名', '未找到可公开核验的同名 supabase-realtime Skill', 'Supabase skill lists Realtime as a trigger', 'auth', 'RLS', 'MCP server'],
        forbidden: /完全免费|Claude Code 内置|postgres_changes 到 broadcast 迁移指南|可扩展模式设计|实时功能专精/,
      },
      'ui-ux-pro-max-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 ui-ux-pro-max skill（检查于 2026-07-02）',
          '本地核验线索：frontend-app-builder',
          '本地核验线索：Product Design ideate',
        ],
        mustInclude: ['公开来源不足', '未找到可公开核验的同名 ui-ux-pro-max Skill', '历史概念', 'frontend-app-builder', 'ideate'],
        forbidden: /完全免费|Claude Code 内置|67 种设计风格|96 种调色板|57 种字体配对|13 种技术栈覆盖|shadcn\/ui MCP 深度集成/,
      },
      'week-report-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 week-report-generator skill（检查于 2026-07-02）',
        ],
        mustInclude: ['公开来源不足', '未找到可公开核验的同名 week-report-generator Skill', '历史概念', '具体内部数据源'],
        forbidden: /完全免费|Claude Code 内置|BigEyes 平台集成|自动化企业周报生成|中文优化到位/,
      },
      'think-harder-skill': {
        status: 'deprecated',
        sources: [
          '公开来源不足：未发现同名 think-harder skill（检查于 2026-07-02）',
          '本地核验线索：brainstorming',
          '本地核验线索：systematic-debugging',
        ],
        mustInclude: ['公开来源不足', '未找到可公开核验的同名 think-harder Skill', '历史概念', 'brainstorming', 'systematic-debugging'],
        forbidden: /完全免费|Claude Code 内置|增强分析推理能力|多角度思考复杂问题|模型自身推理增强/,
      },
      'research-skill': {
        status: 'verified',
        sources: [
          '本地核验线索：Product Design research',
          '本地核验线索：anycap-deepresearch',
        ],
        mustInclude: ['基于来源的 UX 研究', '体验摩擦', 'AnyCap Deep Research', '多来源研究', '引用', '区分观察证据与推断'],
        forbidden: /完全免费|Claude Code 内置|并行 Subagent 深度研究|自动引用来源|比手动搜索效率高出数倍/,
      },
    }
  ...
  it('keeps source-pending audit tokens out of user-facing copy', () => {
    ...
    aiToolsData.forEach((tool) => {
      ...
      expect(visibleText, tool.id).not.toMatch(
        /\bno exact local [^"'，。]+ skill found\b/i,
      )
